...
from src.sync.flags import get_local_flags, load_sync_flags
...
class FlagsManagerDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        info = QLabel(
            ...
            "Check 'Ignore' to never apply on this machine."
        )
        root.addWidget(info)

        table = QTableWidget(len(flag_sources), 3)
        table.setHorizontalHeaderLabels(["Flag", "Source", "Ignore"])
        ...
        for row, (flag, sources) in enumerate(sorted(flag_sources.items())):
            ...
            check = QCheckBox()
            check.setChecked(flag in ignore_set)
            self._ignore_checks[flag] = check
            cell = QHBoxLayout()
    ...
    def _save(self) -> None:
        ignore = [f for f, chk in self._ignore_checks.items() if chk.isChecked()]
